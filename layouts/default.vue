<template>
  <v-app>
    ...
    >
      ...
      <v-toolbar-title
        v-if="reloadEffect"
        class="font-weight-bold v-chip subtitle-2 px-1 hidden-xs-only white--text grey lighten-4"
        style="min-width: 32%; opacity: 0.6"
      >
        <vue-typer
          :text="titleLoad"
          :repeat="0"
      ...
      <v-toolbar-title
        v-else
        class="font-weight-bold v-chip subtitle-2 px-1 hidden-xs-only hidden-sm-only grey darken-3"
        v-html="title"
      ></v-toolbar-title>
            ...
            <v-col cols="12" sm="4" md="4" lg="4">
              <v-card :class="profiles.fullstack_eng.ccClass" flat tile>
                <v-card-text :class="'pa-1 ' + profiles.fullstack_eng.cssClass">
                  ...
                  ></div>
                  <div
                    v-html="profiles.fullstack_eng.statusText"
                    class="caption font-weight-medium font-italic mt-1 mb-1"
                  ></div>
            ...
            <v-col cols="12" sm="4" md="4" lg="4" class="col-xs-12">
              <v-card :class="profiles.workspace_tools.ccClass" flat tile>
                ...
                >
                  ...
                  ></div>
                  <div
                    v-html="profiles.workspace_tools.statusText"
                    class="caption font-weight-medium font-italic mt-1 mb-1"
                  ></div>
                  <v-divider class="mt-2 mb-2"></v-divider>
                  <template v-for="skill in profiles.workspace_tools.skills">
                    <v-chip small :key="skill.name" class="ma-1 transparent">
                      <v-avatar left>
                        <v-img :src="skill.icon"></v-img>
                      </v-avatar>
    ...
    <transition name="assistant" appear>
      ...
      <!-- A voice assistant that handles users input and match them with app actions -->
    </transition>
  </v-app>
</template>
